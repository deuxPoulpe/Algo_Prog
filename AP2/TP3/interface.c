#include "interface.h"
#include "jeu.h"
#define NB_LIGNES_PIECES 2
unsigned char dessin_piece[NB_LIGNES_PIECES][HAUTEUR_CASE][LARGEUR_CASE];
void init_dessins_pieces()
{
    int i, j;
    ...
    {
        dessin_piece[0][i][j] = ' ';
        dessin_piece[0][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    for (j = (LARGEUR_CASE - 3) / 2; j < (LARGEUR_CASE - 3) / 2 + 3; j++)
        dessin_piece[0][i][j] = 219;
    i++;
    for (j = 0; j < (LARGEUR_CASE - 3) / 2; j++)
    {
        dessin_piece[0][i][j] = ' ';
    ...
    dessin_piece[0][i][(LARGEUR_CASE - 3) / 2] = 220;
    dessin_piece[0][i][(LARGEUR_CASE - 3) / 2 + 1] = 219;
    dessin_piece[0][i][(LARGEUR_CASE - 3) / 2 + 2] = 220;
    i++;
    for (j = 0; j < (LARGEUR_CASE); j++)
        dessin_piece[0][i][j] = ' ';
    dessin_piece[0][i][0] = ' ';
    dessin_piece[0][i][LARGEUR_CASE - 1] = ' ';
    ...
    for (i = 0; i < HAUTEUR_CASE - 4; i++)
        for (j = 0; j < LARGEUR_CASE; j++)
            dessin_piece[1][i][j] = ' ';
    for (j = 0; j < (LARGEUR_CASE - 4) / 2 + 1; j++)
    {
        dessin_piece[1][i][j] = ' ';
        dessin_piece[1][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    dessin_piece[1][i][(LARGEUR_CASE - 4) / 2] = 220;
    dessin_piece[1][i][(LARGEUR_CASE - 4) / 2 + 1] = 219;
    dessin_piece[1][i][(LARGEUR_CASE - 4) / 2 + 2] = 223;
    dessin_piece[1][i][(LARGEUR_CASE - 4) / 2 + 3] = 220;
    i++;
    ...
    dessin_piece[1][i][(LARGEUR_CASE - 5) / 2] = 219;
    for (j = (LARGEUR_CASE - 5) / 2 + 1; j < (LARGEUR_CASE - 5) / 2 + 1 + 3; j++)
        dessin_piece[1][i][j] = ' ';
    dessin_piece[1][i][j] = 223;
    i++;
    for (j = 0; j < (LARGEUR_CASE - 3) / 2; j++)
    {
        dessin_piece[1][i][j] = ' ';
    ...
    for (j = (LARGEUR_CASE - 3) / 2 - 1; j < (LARGEUR_CASE - 3) / 2 + 1; j++)
        dessin_piece[1][i][j] = 219;
    dessin_piece[1][i][j] = 220;
    i++;
    for (j = 0; j < (LARGEUR_CASE - 5) / 2; j++)
    {
        dessin_piece[1][i][j] = ' ';
    ...
    {
        dessin_piece[2][i][j] = ' ';
        dessin_piece[2][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    dessin_piece[2][i][j++] = 220;
    dessin_piece[2][i][j++] = 219;
    dessin_piece[2][i][j++] = 219;
    dessin_piece[2][i][j++] = 223;
    dessin_piece[2][i][j++] = ' ';
    ...
    {
        dessin_piece[3][i][j] = ' ';
        dessin_piece[3][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    dessin_piece[3][i][j++] = 219;
    dessin_piece[3][i][j++] = ' ';
    dessin_piece[3][i][j++] = 219;
    dessin_piece[3][i][j++] = ' ';
    dessin_piece[3][i][j++] = 219;
    ...
    {
        dessin_piece[4][i][j] = ' ';
        dessin_piece[4][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    dessin_piece[4][i][j] = 220;
    i++;
    for (j = 0; j < (LARGEUR_CASE - 5) / 2; j++)
    {
        dessin_piece[4][i][j] = ' ';
    ...
    dessin_piece[4][i][j++] = ' ';
    dessin_piece[4][i][j++] = 223;
    dessin_piece[4][i][j++] = 219;
    dessin_piece[4][i][j++] = 223;
    dessin_piece[4][i][j++] = ' ';
    i++;
    for (j = 0; j < (LARGEUR_CASE - 3) / 2; j++)
    ...
    {
        dessin_piece[5][i][j] = ' ';
        dessin_piece[5][i][LARGEUR_CASE - 1 - j] = ' ';
    }
    dessin_piece[5][i][j++] = 219;
    dessin_piece[5][i][j++] = 220;
    dessin_piece[5][i][j++] = 219;
    dessin_piece[5][i][j++] = 220;
    dessin_piece[5][i][j++] = 219;
    i++;
...
deplacement saisie_deplacement(echiquier grid, couleur player_color)
{
    ...
    while (ok == 0)
    {
        ...
        else
        {
            ...
            deplacements_possibles(grid, d1.depart, test_grid);
